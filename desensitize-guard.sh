...
FILES=$(find "$PKG" -type f \
  ...
  -not -path "*/.git/*" -not -name "desensitize-guard.sh" -not -name "desensitize-guard.ps1" -not -name "_sync-check.sh" -not -name "_sync-check.ps1" -not -name "LICENSE" 2>/dev/null)
